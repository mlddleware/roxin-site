...
def orders_view():
    ...
    try:
        ...
        # ИСПРАВЛЕННЫЙ запрос с правильными столбцами
        # customer_orders: id, user_id, tech_assignment, budget, service (в таком порядке)
        cursor.execute("SELECT * FROM customer_orders")
        orders_raw = cursor.fetchall()
        print(f"ДЕБАГ: Найдено заказов в customer_orders: {len(orders_raw)}")

        if not orders_raw:
            print("ДЕБАГ: Заказы не найдены в customer_orders")
        ...
        for order_raw in orders_raw:
            # order_raw: (id, user_id, tech_assignment, budget, service)
            order_id = order_raw[0]
            
            # Получаем статус заказа отдельно
            cursor.execute("SELECT status, created_at, coder, last_from FROM order_status WHERE order_id = %s", (order_id,))
            status_result = cursor.fetchone()
            
            if status_result:
                order_status, order_created_at, coder_id, last_from = status_result
            else:
            ...
            if filter_status:
                if filter_status == "my_orders" and str(last_from) != user_id:
                    continue
                elif filter_status != "my_orders" and filter_status != order_status:
                    continue

            cursor.execute("SELECT username FROM users WHERE id = %s", (order_raw[1],))
            user_result = cursor.fetchone()
            username = user_result[0] if user_result else "Неизвестный пользователь"

            # Получаем имя кодера если есть
            coder_name = "Не назначен"
            if coder_id:
                cursor.execute("SELECT username FROM users WHERE id = %s", (coder_id,))
                coder_result = cursor.fetchone()
                coder_name = coder_result[0] if coder_result else "Неизвестный кодер"
